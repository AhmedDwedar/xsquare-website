...
// lang button
$(".ar").click(function(){
    $("#hhead").text("اتبيع أونلاين أو تحب تبدأ مشروعك الخاص!");
    $("#phead").text("نوفر لك المنتجات تحت التصريف والدفع بعد البيع و التوصيل لعميلك مباشرة");
    $("#hteam").text("وقت التسويق نحن الى جانبك");
    $("#pteam").text("لن ننساك! سوف نقوم بتوفير دروس مكتوبة لك لكيفية القيام بالتسويق الإلكتروني عبر وسائل التواصل الاجتماعي وأفكار تسويقية واذا لديك حملات نساعدك بنصائح تسويقية لنجاحها، بالإضافة الى دورات مباشرة أونلاين بين فترة وأخرى لتطوير مهاراتك.");
    $("#hpay").text("ما تحتاج رأس مال كبير!");
    $("#ppay").text("حين يتسوق الناس في متجرك او في أي مكان تعرض فيه منتجاتك ويقومون بالشراء فكل ما عليك هو دفع التكلفة فقط والاحتفاظ بالباقي، كل ما تحتاجه فقط قيمة الاشتراك لقنوات وتصميم متجر بسيط وشوية تسويق.");
    $("#hstor").text("ما عندك مكان للبيع؟ ما تشيل هم!");
    $("#pstor").text("اربط متجرك بكل سهوله او اذا ليس لديك صمم متجر خاص باستخدام نظام ووكومرس المجاني او باشتراك شهري من shopify او salla.sa  اعرض منتجاتك في متجرك وفي الانستجرام والفيسبوك وسناب شات وتويتر وأمازون وإيباي وأي مكان يعجبك، المهم تكون متواجد في كل مكان لتصل لعملاء أكثر وتبيع أكثر وتربح أكثر.و غيرها");
    $("#haccess").text("سعر القطعة بسعر الجملة");
    $("#paccess").text("كل منتج تبيعه انت تحصل عليه بسعر الجملة بدون التزام بشراء كميات أبدا أو تدفع مقدما أو أي وجع رأس. نحن في قنوات نختار الموردين بعناية ونفحص المنتجات ونتفاوض لنعطيك أفضل أسعار الجملة عشان أرباحك تتضاعف.");
    $("#hprise").text("ما تحتاج رأس مال كبير!");
    $("#pprise").text("حين يتسوق الناس في متجرك او في أي مكان تعرض فيه منتجاتك ويقومون بالشراء فكل ما عليك هو دفع التكلفة فقط والاحتفاظ بالباقي، كل ما تحتاجه فقط قيمة الاشتراك لقنوات وتصميم متجر بسيط وشوية تسويق.");
        $("#huser").text("منتجاتنا بلغتك");
        $("#puser").text("الوصف والمحتوى باللغة العربي والانجليزية جاهزة ويتم اضافتها لمتجرك دون ما تحتاج لمترجمين أو كاتبين وصف منتجات أو موظفين مراجعة، خليك مرتاح وقنوات بتخدمك.");
        $("#follow").text("تابعونا علي");
        $("#subscribe").text("شاركنا كل ما هو جديد");
    $("#sitemap1").text("قنوات");
      $("#home").text("الرئيسية");
      $("#feature").text("الميزات");
      $("#pricing").text("الاسعار");
      $("#blog").text("المدونه");
    $("#company1").text("الشركه");
        $("#terms").text("الشروط و الاحكام");
        $("#privacy").text("الخصوصيه و الدعم");
        $("#fees").text("رسوم و الاعاده");
        $("#contactus").text("اتصل بنا");
    $("#community1").text("المجتمع");
        $("#program").text("مركز الدعم");
        $("#help").text("الاسئله الاكسر شيوعا");
        $("#faq").text("اعمل معنا");
    $("#integration1").text("الربط");
        $("#developers").text("المطوريين")
        $("#woo").text("وكومرس");
        $("#magento").text("ماجنيتوا");
        $("#shopify").text("شوبيفاي");
    $("#language1").text("اللغه");
    //li line
    $("#hsvg").text("شركتنا ببساطة");
    $(".hsell").text("لا حدود تفصلك عن عملائك");
    $(".hearn").text("أنت تقرر قيمة أرباحك");
    $(".htrue").text("تحصل على كامل الدعم");
    $(".hquality").text("جودة الخدمة المقدمة");
    $(".hmulti").text("محتوى متعدد اللغات");
    $(".hstore").text("الربط بمتجرك الإلكتروني");
 
    $(".psell").text("نقوم بإيصال طلبات عملائك إلى أي مكان في العالم.");
    $(".pearn").text("محتوى النظام لدينا يدعم مجموعة متعددة من اللغات (التركية، العربية، الإنجليزية، الفرنسية)");
    $(".ptrue").text("سوف تحصل على دروس ومحاضرات تدعمك في عملية البيع.");
    $(".pquality").text("لدينا فريق مؤهل ومتكامل يهتم بتنفيذ العمليات وتقديم الدعم لعملائنا بأعلى");
    $(".pmulti").text("لن يكون عليك البيع بسعر محدد، أنت تقرر الأسعار وتضع هامش الربح الذي يناسبك.");
    $(".pstore").text("نقوم بعملية ربط متجرك الإلكتروني بقاعدة بياناتنا التي تحوي حتى 100 ألف منتج، وتختار ما يلائمك منها بالمجان.");
    $(".right").addClass("rtl");
